...
describe("SupabaseArtistSimilarityGraphStore canonical identity", () => {
  it("upserts deterministic artist observations with their event provenance", async () => {
    ...
    await store.recordArtistObservation("artist-1", {
      role: "lineup_artist",
      sourceProvider: "ticketmaster",
      sourceUrl: "https://tickets.example/event-1",
      eventExternalId: "event-1",
      eventName: "Headliner + Support",
      venueName: "Le Klub",
      confidence: 0.8,
      observedAt: "2026-09-19T12:00:00.000Z"
    });

    expect(String(fetchMock.mock.calls[0]?.[0])).toContain("global_artist_observations?on_conflict=observation_key");
    const body = JSON.parse(String(fetchMock.mock.calls[0]?.[1]?.body));
    expect(body).toMatchObject({
      artist_id: "artist-1", role: "lineup_artist", source_provider: "ticketmaster",
      event_external_id: "event-1", event_name: "Headliner + Support", venue_name: "Le Klub"
    });
    expect(body.observation_key).toMatch(/^[a-f0-9]{64}$/);
