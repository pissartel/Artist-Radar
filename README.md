...
- `ENABLE_FIRECRAWL_CONSOLIDATION=true` plus `FIRECRAWL_API_KEY` enables Firecrawl search/markdown enrichment during consolidation. `DEBUG_FIRECRAWL=true` logs key presence, generated queries, result counts, selected URLs and extracted evidence without logging the key.
