...
export interface ConcertSearchMarket {
  label: string;
  country?: string | null;
}

/** Deterministic — no randomness, so the same artist/window always produces the same prompt (needed for stable caching). */
export function buildConcertSearchPrompt(
  artist: ConcertSearchArtistIdentity,
  windows: ConcertDateWindows,
  market?: ConcertSearchMarket | null
): string {
  const identityLines = [
    `- Name: ${artist.name}`,
...
Target search market:
${market?.label ? `- Search location: ${market.label}` : "- Search location: not specified"}
${market?.country ? `- Required country when a country is reported: ${market.country}` : "- Required country: unknown"}
...
For every event, extract: event name, date, venue name, city, region, country, official venue website when available, lineup when available, event type, status, source URLs, source titles, source type, and a concise evidence summary.

Rules:
- Only return events supported by at least one accessible web source.
- Search primarily inside the target search market.
- Do not return concerts from unrelated countries when a required country is provided.
